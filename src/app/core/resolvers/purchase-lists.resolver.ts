...
import { defaultIfEmpty, forkJoin, Observable, of, switchMap, tap } from 'rxjs';
...
export const purchaseListResolver: ResolveFn<PurchaseListModel[]> = (route, state) => {
  ...
  return purchaseListService.getUserPurchaseLists$().pipe(
    tap(response => purchaseListService.purchaseLists$.next(response)),
