...
import { ProductBaseDto } from '../../../../core/dtos/product-base.dto';
...
export class ProductBaseListComponent implements OnDestroy {
  ...
  productBases = signal<ProductBaseDto[]>([]);
  ...
  constructor() {
    this._activatedRoute.paramMap.pipe(takeUntil(this._unsubscribe)).subscribe(() => {
      const pageProductBases: PageDto<ProductBaseDto> = this._activatedRoute.snapshot.data['pageProductBases'];
  ...
  private setPageProductBase(pageProductBases: PageDto<ProductBaseDto>): void {
